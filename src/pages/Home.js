  ...
  return (
    ...
    {/* {newArrivals} */}
      ...
      <div className="headding-container">
        ...
        <Carousel containerStyle={{"width":"100%"}} loop={false} cols={4} rows={1}  responsiveLayout={
                    [
                        {
                          gap:10 ,
                          cols: 2,
...
// console.log(newArrivalArray);
              return (newArrivalArray.filter(name => name.includes('NEW_ARRIVALS'))=='NEW_ARRIVALS')?
              (
             <Carousel.Item key={index}>
      <Card  image={imagePath + item.product_img} cardClass="single-card"  productname={item.product_name} oldprice={item.old_price}
       newprice={item.selling_price} description={item.description} />
            </Carousel.Item>
              )
         :(null)
          })
                ...
                } >
        {
          ...
          var newArrivalArray= JSON.parse(item.product_tags);
// console.log(newArrivalArray);
              return (newArrivalArray.filter(name => name.includes('BEST_SELLER'))=='BEST_SELLER')?
              (
             <Carousel.Item key={index}>
      <Card  image={imagePath + item.product_img} cardClass="single-card"  productname={item.product_name} oldprice={item.old_price}
       newprice={item.selling_price} description={item.description} />
